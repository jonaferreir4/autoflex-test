...
export function ProductCard({ product, materials, onEdit, isExpanded, onToggle }: Props) {
  ...
  return (
    <div 
      className={`
        bg-white p-4 rounded-lg shadow-md border transition-all duration-200
        flex flex-col justify-between
        w-full                  
        min-h-[200px]
        ${isExpanded ? 'border-blue-400 ring-2 ring-blue-100 h-auto' : 'border-gray-200 hover:shadow-lg'}
      `}
    >
      <div className="flex justify-between items-start mb-4 flex-1">
        <div className="flex-1 pr-2"> {/* pr-2 evita que texto encoste nos botões */}
          <div className="flex flex-col gap-1">
            <span className="text-xs bg-gray-100 text-gray-500 px-2 py-0.5 rounded font-mono w-fit">
              {product.code}
            </span>
            <h3 className="font-bold text-lg text-gray-800 leading-tight line-clamp-2" title={product.name}>
              {product.name}
            </h3>
          </div>
          <p className="text-green-600 font-semibold mt-2">R$ {product.value.toFixed(2)}</p>
        </div>

        <div className="flex gap-1 shrink-0">
          <button onClick={(e) => { e.stopPropagation(); onEdit(product); }} className="p-1.5 text-blue-600 hover:bg-blue-50 rounded" title="Editar">
            <Pencil size={16} />
          </button>
          <button onClick={(e) => { e.stopPropagation(); handleDeleteProduct(); }} className="p-1.5 text-red-600 hover:bg-red-50 rounded" title="Excluir">
            <Trash2 size={16} />
          </button>
        </div>
      </div>
